...
const Departments = db.doctor_department;
...
// Retrieve all doctor_department from the database.
exports.findAll = (req, res) => {
  ...
  Departments.findAll({ where: condition })
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving doctor_department."
      });
    });
};
...
// Delete all doctor_department from the database.
exports.deleteAll = (req, res) => {
  Departments.destroy({
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while removing all doctor_department."
      });
    });
};
